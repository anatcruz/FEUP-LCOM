...
uint16_t get_bytes_per_pixel() {return bytes_per_pixel;}
uint16_t get_buffer_size() {return hres * vres * bytes_per_pixel;}
vbe_mode_info_t get_inf_memory() {return inf_memory;}
uint8_t* get_buffer_base_ptr() {return (uint8_t*) video_mem;}
...
void clean_screen() {
  memset(video_mem, 0, hres * vres * bytes_per_pixel);
}

//We don't actually use these functions, they are just here to show that we can implement them
int (vg_draw_hline) (uint16_t x, uint16_t y, uint16_t len, uint32_t color) {
  set_rectangle((uint8_t*) video_mem, x, y, len, 1, color);
  return 0;
}

int (vg_draw_rectangle) (uint16_t x, uint16_t y,	uint16_t width,	uint16_t height, uint32_t color) {
    set_rectangle((uint8_t*) video_mem, x, y, width, height, color);
    return 0;
}
